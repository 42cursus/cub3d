...
void	draw_ring_segment(t_img *img, t_ring_segment segment, int color)
{
	...
	y = -segment.r_outer - 1;
	while (++y <= segment.r_outer)
	{
		x = -segment.r_outer - 1;
		while (++x <= segment.r_outer)
		{
			fx = x + 0.5;
			fy = y + 0.5;
			dist = sqrt(fx * fx + fy * fy);
			if (dist < segment.r_inner - 1.0 || dist > segment.r_outer)
				continue;
			...
			if (angle_in_range(angle, segment.angle_start, segment.angle_end))
			{
				// Angular blur (soft arc boundary)
				double da_start = angle - segment.angle_start;
				double da_end   = segment.angle_end - angle;
				...
				alpha = 1.0;
				if (dist < segment.r_inner)
					alpha = dist - (segment.r_inner - 1.0);
				else if (dist > segment.r_outer - 1.0)
					alpha = segment.r_outer - dist;

				alpha = 1 - fmin(alpha, fmin(a_edge_start, a_edge_end));

				t_point cc = { .x = segment.center.x + x, .y = segment.center.y + y };
...
{
	double angle_start = -app->player->angle - 2 * M_1_PI;
	double angle_end = -app->player->angle + M_1_PI;

	normalize_angles(&angle_start, &angle_end);

	t_ring_segment segment = {
		.center = (t_point){ .x = app->pointer->width / 2, .y = app->pointer->height / 2},
		.angle_start = angle_start,
		.angle_end = angle_end,
		.r_inner = 7, .r_outer = 20
	};
	fill_with_colour(app->pointer, XPM_TRANSPARENT, XPM_TRANSPARENT);
