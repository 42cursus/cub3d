...
int	bilinear_filter(double x, double y, const t_texture *tex)
{
	...
	if (x_upper == tex->x)
		x_upper = 0;
	if (y_upper == tex->y)
		return (interpolate_colour_inline(tex->data[y_lower * tex->x + x_lower], tex->data[y_lower * tex->x + x_upper], frac_x));
	int	interp_x1 = interpolate_colour_inline(tex->data[y_lower * tex->x + x_lower], tex->data[y_lower * tex->x + x_upper], frac_x);
	int	interp_x2 = interpolate_colour_inline(tex->data[y_upper * tex->x + x_lower], tex->data[y_upper * tex->x + x_upper], frac_x);
	return (interpolate_colour_inline(interp_x1, interp_x2, frac_y));
}
